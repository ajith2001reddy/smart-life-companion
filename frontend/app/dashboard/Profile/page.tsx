...
export default function ProfilePage() {
    ...
    async function handleUpdate() {
        ...
        try {
            ...
            setMsg(res.ok ? { text: "Profile updated successfully!", ok: true } : { text: data.error || "Update failed", ok: false });
    ...
    if (!profile)
        return (
            <div className="flex items-center justify-center h-64">
                <motion.div className="w-12 h-12 border-2 border-[#c8ff00] border-t-transparent rounded-full" animate={{ rotate: 360 }} transition={{ duration: 0.8, repeat: Infinity, ease: "linear" }} />
            </div>
        );
            ...
            <motion.div variants={fadeUp} className="backdrop-blur-2xl bg-white/5 border border-white/10 rounded-3xl p-5 sm:p-8">
                <div className="flex flex-col sm:flex-row sm:items-center gap-5">
                    <div className="relative">
                        <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-[#c8ff00] flex items-center justify-center text-black text-xl sm:text-2xl font-bold select-none">
                            {initials}
                        </div>
                        <motion.div className="absolute -bottom-1 -right-1 w-4 h-4 bg-[#c8ff00] rounded-full border-2 border-black" animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 2, repeat: Infinity }} />
                    </div>
                ...
                {activeTab === "account" && (
                    <motion.div key="account" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                        ...
                        <div className="backdrop-blur-2xl bg-white/3 border border-white/8 rounded-3xl p-5 sm:p-8">
                            ...
                            <p className="text-white/30 text-sm mb-4">Use this ID in your Apple Shortcut to sync health data to your account.</p>
                    ...
                    <motion.div key="goals" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                        <div className="backdrop-blur-2xl bg-white/5 border border-white/10 rounded-3xl p-5 sm:p-8 space-y-6">
                            <div>
                                ...
                                <p className="text-white/40 text-sm">Set your targets — these are used by the AI Coach and dashboard insights.</p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                {[
                                    { key: "weeklySteps", label: "Weekly Steps Target", placeholder: "70000", unit: "steps", min: 10000, max: 200000, step: 5000 },
                                    { key: "dailyCalories", label: "Daily Calorie Target", placeholder: "2200", unit: "kcal", min: 1200, max: 5000, step: 100 },
                                    { key: "sleepTarget", label: "Sleep Target", placeholder: "8", unit: "hrs/night", min: 5, max: 12, step: 0.5 },
                                    { key: "weightTarget", label: "Weight Target (optional)", placeholder: "e.g. 75", unit: "kg", min: 0, max: 300, step: 0.5 },
                                ].map((field) => (
                                    <div key={field.key}>
                                        ...
                                        <input
                                            ...
                                            max={field.max}
                                            step={field.step}
                                            className="w-full px-5 py-3 rounded-2xl bg-white/5 border border-white/10 text-white outline-none focus:border-[#c8ff00]/50 transition text-sm placeholder:text-white/20"
                                        />
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="space-y-3 pt-2 border-t border-white/8">
                                <p className="text-xs text-white/30 uppercase tracking-widest">Target Summary</p>
                                {[
                            ...
                            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={saveGoals} className="bg-[#c8ff00] text-black px-8 py-3 rounded-2xl font-bold text-sm">
                                {goalSaved ? "✓ Goals Saved!" : "Save Goals"}
                            </motion.button>
                        </div>
                    </motion.div>
                )}
                    ...
                    <motion.div key="notifications" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                        <div className="backdrop-blur-2xl bg-white/5 border border-white/10 rounded-3xl p-5 sm:p-8 space-y-6">
                            <div>
                                ...
                                <p className="text-white/40 text-sm">Choose what alerts you want Smart Life to surface.</p>
                            </div>

                            <div className="space-y-1">
                                {([
                                    { key: "dailyReminder", label: "Daily Training Reminder", desc: "Get a nudge each morning to log your activity", icon: "⏰" },
                                    { key: "weeklyReport", label: "Weekly Progress Report", desc: "Summary of your week every Sunday", icon: "📊" },
                                    { key: "recoveryAlerts", label: "Recovery Alerts", desc: "Notified when your recovery score drops below 60", icon: "😴" },
                                    { key: "goalAchievements", label: "Goal Achievements", desc: "Celebrate when you hit your daily or weekly targets", icon: "🏆" },
                                ] as const).map((item, i) => (
                                    <motion.div
                                        key={item.key}
                    ...
                    <motion.div key="activity" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                        <div className="backdrop-blur-2xl bg-white/5 border border-white/10 rounded-3xl p-5 sm:p-8 space-y-5">
                            <div>
                                ...
                                <p className="text-white/40 text-sm">A summary of recent actions and data in your account.</p>
                            </div>
                            ...
                            <div className="pt-4 border-t border-white/8 grid grid-cols-2 sm:grid-cols-4 gap-3">
                                {[
                                    ...
                                    { label: "Goals Set", value: localStorage.getItem("smartlife-goals") ? 1 : 0, icon: "🎯" },
                                ].map((stat) => (
